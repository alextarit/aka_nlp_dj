...
## 📚 Краткое описание как выполнены критерии каждой из лаб

- [x] Lab 1: Простой и надежный workflow 

    - ReAct паттерн - реализован в subgraph `lyrics`
    - Инструменты - Интеграция с Genius API и RAG `(SOON: LAB 2)`
    - SO (structured output) - реализован c помощью Pydantic models
    - Повторные попытки, реализованы с помощью `loop_count` (это условное ребро у react) + ноды основного графа имеют RetryPolicy дефолтные 
    - Разделение шагов - разные сабграфы, разные ноды - нет толстых реализаций
    - `Не используем pydantic models для состояния/каналов` - хочу кастомности редюсеров на всякий случай + под капотом все равно работаем с dict
    - `Есть 20% вайбкодинга, но хотя бы не Карпаты с LLM консилиум - времени жалко, но архитекутура местами страдает кнч`
...
Этот репозиторий разрабатывается по лучшим практикам:  
- **Современный Python** (>=3.13), строгая типизация
- **Менеджер пакетов [uv](https://uv.pyfound.org/)**: быстрая установка и reproducible environment
- **[pre-commit](https://pre-commit.com/)**: автоформатирование, автоисправление стиля и lint (никакой грязи в diff!)  
- **[ruff](https://docs.astral.sh/ruff/)** – быстрый и строгий линтер/форматер  
  > Автоматически подчищает стиль, fix'ит импорты, держит код в чистоте.

**Архитектура приложения** — слоистая (чистая domain-driven архитектура):  
- 🚀 **SOLID** — максимальное разделение ответственности между слоями
- 🔁 **DRY** — минимизация дублирования, вынос логики в общие утилиты, токенизация задач  
- 🏗️ **Расширяемость:** новый workflow, новую модель или провайдер легко добавить отдельным модулем без переписывания основного ядра.

_TODO: Хотелось бы, но пишем в рилтайме, без рефлексий, бизнесанализа, системного анализа и прочей нечисти_

> Наш приоритет — осознанная инженерия, не жертвуем качеством ради скорости, но и не уходим в оверинжиниринг:  
Чистый, читаемый, поддерживаемый код даже в учебных проектах.

_Кто понял — тот понял. Welcome to the code!_
...
### Пример 1: Реп-трек про классиков русской литературы

**Запрос:**
```
Напиши в стиле Dragonborn от музыканта Big Baby Tape песню касательно русской литературы стиль реп,треп и 808
```

**Результат:** [song_example_1.mp3](docs/song_example_1.mp3)

В этом примере система создала реп-композицию в стиле Big Baby Tape с упоминанием университета значимых лиц русской литературы, используя характерные для трепа и репа 808-биты.
